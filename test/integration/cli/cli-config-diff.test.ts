// RE-ARCH: keep
/**
 * Config Command --diff integration tests
 *
 * Covers project-vs-global diff output and related edge cases for `nax config --diff`.
 */
...
describe("Config Command --diff", () => {
  ...
  describe("Project config overrides global", () => {
    ...
    test("handles boolean differences correctly", async () => {
      ...
      const projectConfig = {
        quality: {
          autofix: { enabled: false }, // Different from default/global (true)
        },
      };
      writeFileSync(join(naxDir, "config.json"), JSON.stringify(projectConfig, null, 2));
      ...
      expect(output).toContain("quality.autofix.enabled");
