...
import { parseDispositions, parseReviewReport } from "@/finish";
...
describe("parseReviewReport", () => {
  ...
  test("parses a section heading that follows narration with a real newline", () => {
    // The legitimate case GLUED_HEADING existed for: the narration message
    // ends without a trailing newline, but message assembly still inserts one
    // before the next message, so the heading lands on its own line and
    // parses normally with no special-casing needed.
    const r = parseReviewReport(
      "No defects cleared the confidence bar.\n## TOUCHPOINTS\n- a.ts:sym — why\n\n## WALK\nb.ts:fn — earns its place\n\n## FINDINGS\nNo findings.\n",
    );
    expect(r.sawTouchpointsSection).toBe(true);
