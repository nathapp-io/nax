 ...
 *      exactly once.
 * AC7: complete() with a response carrying `exactCostUsd` propagates both
 *      `pricingSource` (from the card) and `exactCostUsd` on CompleteResult —
 *      the cost middleware later stamps "wire" because exactCostUsd wins
 *      over a producer-supplied pricingSource.
 *
...
describe("complete() — pricingSource from resolveRateCard", () => {
  ...
  // AC7 (success): when the response carries wire-reported exactCostUsd,
  // the adapter still propagates the card's source onto pricingSource.
  // The middleware overwrites to "wire" at the cost-row level — that is
  // the precedence AC7 names — but the producer (adapter) does not
  // strip the field; the dispatch event still carries the card's
  // source, and the middleware uses exactCostUsd to choose "wire"
  // instead.
  test("AC7: complete() with wire-reported exactCostUsd still stamps pricingSource from the card on CompleteResult", async () => {
    const card = stubResolveRateCard(CATALOG_CARD);
    _acpAdapterDeps.resolveRateCard = mock(card.fn);
...
describe("openSession() + sendTurn() — pricingSource from resolveRateCard", () => {
  ...
  // AC7 (sendTurn side): the same wire-precedence rule applies on the
  // session-turn path — the card's source still reaches TurnResult, and
  // the dispatch cost row will stamp "wire" because exactCostUsd wins.
  test("AC7: sendTurn() with wire-reported exactCostUsd still stamps pricingSource from the card on TurnResult", async () => {
    const card = stubResolveRateCard(CATALOG_CARD);
    _acpAdapterDeps.resolveRateCard = mock(card.fn);
    ...
    const adapter = new AcpAgentAdapter("claude");
    const handle = await adapter.openSession("nax-rate-card-wire", makeOpenSessionOpts());
    const result = await adapter.sendTurn(handle, "do the thing", { interactionHandler: NO_OP_INTERACTION_HANDLER });
    expect(result.exactCostUsd).toBe(0.012);
    expect(result.pricingSource).toBe("catalog-rates");
  });
