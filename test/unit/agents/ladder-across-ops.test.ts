 ...
 *
 * The composite suite is nax#1965's final-review fix: the three-operation suite
 * proves `SessionManager.decideReuse` is correct GIVEN a hand-supplied model
 ...
 * `resolveDispatchTarget` / `recordDispatchOutcome` slot path. It does not prove
 * anything CHOSE that sequence. The composite drives the same
 * "native A -> native B -> native C across two operations of one story" scenario
 * through `callOp`, so every decision is made by the real stack: `AgentManager`
 * (real: `nextCandidate`, depth resolution, `CooldownStore`), the configured
 * `agent.fallback.map` ladder (real: `resolveFallbackDispatchTarget` /
 * `resolveFallbackModelId`), `SessionManager` (real, via `createRuntime`'s
 * default: `decideReuse`), and the slot read/write path (real:
 * `resolveDispatchTarget` / `recordDispatchOutcome` / `ladderSlotFor` /
 * `recordLadderSlot`). The ONLY stub is the adapter's underlying dispatch
 * (`openSession` / `sendTurn`) — scripted: rate-limit on native A, rate-limit on
 * native B, success on native C — recording the endpoint it was actually asked
 * to dispatch the same way `recordingAdapter()` does.
 */
