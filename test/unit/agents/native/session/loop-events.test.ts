...
describe("loop events — after_tool dispatcher", () => {
  test("AC1: with no registered handler the payload content and isError are unchanged", () => {
    const registry = createLoopEventRegistry();
    const payload: AfterToolPayload = { content: "raw tool output", isError: false };
    const patched = registry.afterTool(CALL, payload);
    expect(patched.content).toBe("raw tool output");
    expect(patched.isError).toBe(false);
  });

  test("AC1 (boundary): one registered handler that returns no patch is a no-op", () => {
    const registry = createLoopEventRegistry();
    const handler: AfterToolHandler = () => ({});
    registry.registerAfterTool(handler);
    const patched = registry.afterTool(CALL, { content: "x", isError: false });
    expect(patched.content).toBe("x");
    expect(patched.isError).toBe(false);
  });

  test("AC2: a second registered handler receives the first handler's returned content", () => {
    const registry = createLoopEventRegistry();
    registry.registerAfterTool(() => ({ content: "first-handler-output" }));
    let observed: string | undefined;
    registry.registerAfterTool((_call, payload) => {
      observed = payload.content;
      return { content: `${payload.content} + second` };
    });
    const patched = registry.afterTool(CALL, { content: "original", isError: false });
    expect(observed).toBe("first-handler-output");
    expect(patched.content).toBe("first-handler-output + second");
  });

  test("AC3: a throwing handler is skipped and the next handler still applies", () => {
    const registry = createLoopEventRegistry();
    const handlerCalls: string[] = [];
    registry.registerAfterTool(() => {
  ...
  test("AC3 (boundary): a throwing handler that is the LAST handler still does not surface the error", () => {
    const registry = createLoopEventRegistry();
    registry.registerAfterTool(() => {
      throw new Error("only handler exploded");
    });
    // Must not throw, must return the unchanged payload.
    const patched = registry.afterTool(CALL, { content: "untouched", isError: false });
    expect(patched.content).toBe("untouched");
    expect(patched.isError).toBe(false);
  });

  test("AC4: when a handler returns only content, the patched payload has no denied field", () => {
    ...
    // one.
    const registry = createLoopEventRegistry();
    registry.registerAfterTool(() => ({ content: "patched" }));
    const patched = registry.afterTool(CALL, { content: "raw", isError: false });
  ...
  test("AC4 (boundary): a handler may not set denied, even when it tries", () => {
    ...
    registry.registerAfterTool(mock(() => ({ content: "patched", denied: { reason: "ha", breach: true } })));
    const patched = registry.afterTool(CALL, { content: "raw", isError: false });
    expect(patched.content).toBe("patched");
...
describe("loop events — before_tool dispatcher", () => {
  test("AC5: before_tool allow with an input returns it as the new input on the outcome", () => {
    const registry = createLoopEventRegistry();
    const outcome = registry.beforeTool(CALL, []);
    expect(outcome.kind).toBe("allow");
  });

  test("AC5: before_tool allow with a rewritten input surfaces the rewrite", () => {
    const registry = createLoopEventRegistry();
    registry.registerBeforeTool(() => ({ kind: "allow", input: { path: "b.ts" } }));
    const outcome: BeforeToolOutcome = registry.beforeTool(CALL, []);
    expect(outcome.kind).toBe("allow");
    if (outcome.kind !== "allow") throw new Error("unreachable");
    expect(outcome.input).toEqual({ path: "b.ts" });
  });

  test("AC5: handlers chain in registration order — the LAST allow wins", () => {
    const registry = createLoopEventRegistry();
    registry.registerBeforeTool(() => ({ kind: "allow", input: { path: "first-rewrite" } }));
    registry.registerBeforeTool(() => ({ kind: "allow", input: { path: "second-rewrite" } }));
    const outcome = registry.beforeTool(CALL, []);
    if (outcome.kind !== "allow") throw new Error("unreachable");
    expect(outcome.input).toEqual({ path: "second-rewrite" });
  });

  test("AC5 (boundary): an allow with no input leaves the call's input untouched", () => {
    const registry = createLoopEventRegistry();
    const handler: BeforeToolHandler = () => ({ kind: "allow" });
    registry.registerBeforeTool(handler);
    const outcome = registry.beforeTool(CALL, []);
    if (outcome.kind !== "allow") throw new Error("unreachable");
    expect(outcome.input).toBeUndefined();
  });

  test("AC6: before_tool block short-circuits the chain and surfaces the handler's content", () => {
    const registry = createLoopEventRegistry();
    registry.registerBeforeTool(() => ({ kind: "block", content: "blocked by policy" }));
  ...
  test("AC7: before_tool terminate is a single decision — one block of content, not chained", () => {
    const registry = createLoopEventRegistry();
    registry.registerBeforeTool(() => ({ kind: "terminate", content: "turn ended" }));
    let secondRan = false;
    registry.registerBeforeTool(() => {
      secondRan = true;
      return { kind: "allow" };
  ...
  test("AC8: before_tool nudge carries the handler text on the outcome", () => {
    const registry = createLoopEventRegistry();
    registry.registerBeforeTool(() => ({ kind: "nudge", text: "you already asked this" }));
    const outcome = registry.beforeTool(CALL, []);
  ...
  test("AC8 (boundary): a throwing before_tool handler is skipped and the next handler decides", () => {
    const registry = createLoopEventRegistry();
    registry.registerBeforeTool(() => {
      throw new Error("first before_tool exploded");
    });
    registry.registerBeforeTool(() => ({ kind: "block", content: "blocked by next handler" }));
    // The throw must not propagate: the dispatcher catches it.
    const outcome = registry.beforeTool(CALL, []);
    expect(outcome.kind).toBe("block");
    if (outcome.kind !== "block") throw new Error("unreachable");
