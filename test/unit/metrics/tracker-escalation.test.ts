...
import {
  ...
  makeMockRuntime,
  makeNaxConfig,
  makePRD as makeSpendPRD,
  makeStory as makeSpendStory,
  makeTestContext,
} from "@test/helpers";
...
const spendAgg = (runId: string): CostAggregator => {
  const agg = new CostAggregator(runId, "/tmp/drain");
  agg.record({
    ts: Date.now(),
    runId,
    agentName: "claude",
 ...
 */
const makeSpendCtx = ({ runtime }: { runtime: NaxRuntime }): PipelineContext =>
  Object.assign(
    makeTestContext({
      story: makeSpendStory({ id: "US-001", title: "Test Story" }),
      prd: makeSpendPRD({ feature: "test-feature", project: "test", branchName: "main" }),
...
describe("StoryMetrics spend shape (#1960)", () => {
  test("cost folds failed-dispatch spend and errorCostUsd carries the failed half", async () => {
    ...
    const metric = await collectStoryMetrics(makeSpendCtx({ runtime }), new Date().toISOString());
  ...
  test("errorCostUsd is absent when nothing threw", async () => {
    ...
    const metric = await collectStoryMetrics(
      makeSpendCtx({ runtime: makeMockRuntime({ costAggregator: agg }) }),
