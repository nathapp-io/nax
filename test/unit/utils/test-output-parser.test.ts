...
import { type TestFailure, formatFailureSummary, parseTestOutput } from "@/test-runners";
...
describe("formatFailureSummary", () => {
  ...
  test("handles failure without stack trace and includes only first of multiple stack lines", () => {
    const noStack: TestFailure[] = [
      { file: "test/nostack.test.ts", testName: "no stack", error: "Error without stack", stackTrace: [] },
    ];
