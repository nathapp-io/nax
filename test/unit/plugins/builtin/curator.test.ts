...
// ---------------------------------------------------------------------------
// Collector→heuristics seam. Absorbed from curator-seam.test.ts.
// ---------------------------------------------------------------------------
...
async function runOnce(root: string, feature: string, runId: string, file: string, when: string, projectKey = "p") {
  ...
  const ctx = makeSeamContext(root, feature, runId, Date.parse(when) - 60_000, projectKey);
...
describe("curator plugin — end to end", () => {
  async function executeRun(
    ...
    const ctx = {
      ...makeSeamContext(root, feature, runId, Date.parse(when) - 60_000, projectKey),
      config: makeNaxConfig({
        curator: {
          enabled: true,
          thresholds: {
