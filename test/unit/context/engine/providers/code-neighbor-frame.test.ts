...
/** size-cap suite (GROWTH-2) — repo-rooted story, no resolvedTestPatterns. */
function makeSizeCapRequest(overrides: Partial<ContextRequest> = {}): ContextRequest {
  return {
    storyId: "US-001",
    repoRoot: "/repo",
    packageDir: "/repo",
...
let orig: {
  ...
  fileSize: typeof _codeNeighborDeps.fileSize;
  detectLanguage: typeof _codeNeighborDeps.detectLanguage;
  getLogger: typeof _codeNeighborDeps.getLogger;
};
...
beforeEach(() => {
  orig = {
    ...
    fileSize: _codeNeighborDeps.fileSize,
    detectLanguage: _codeNeighborDeps.detectLanguage,
    getLogger: _codeNeighborDeps.getLogger,
  };
});
...
afterEach(() => {
  ...
  _codeNeighborDeps.fileSize = orig.fileSize;
  _codeNeighborDeps.detectLanguage = orig.detectLanguage;
  _codeNeighborDeps.getLogger = orig.getLogger;
});
...
// GROWTH-2: a candidate reverse-dep file over MAX_NEIGHBOR_FILE_SIZE_BYTES is
// skipped before its full content is read into the shared cache. The
...
describe("CodeNeighborProvider — size cap (GROWTH-2)", () => {
  test("skips reading a candidate reverse-dep file over the size cap", async () => {
    ...
    const provider = new CodeNeighborProvider();
    await provider.fetch(makeSizeCapRequest({ touchedFiles }));
