...
// ─────────────────────────────────────────────────────────────────────────────
// paths: frontmatter package-scope filter (#561) and #558 differentiated
// empty-merge log messages — absorbed from static-rules-paths.test.ts
// ─────────────────────────────────────────────────────────────────────────────

const PATHS_BASE_REQUEST: ContextRequest = {
  storyId: "US-001",
  repoRoot: "/project",
  packageDir: "/project",
  stage: "execution",
...
function setupPathsCanonical(repoRules: CanonicalRule[], packageRules: CanonicalRule[] = []) {
...
describe("StaticRulesProvider — paths: frontmatter package-scope filter", () => {
  test("rule with no paths: applies globally in monorepo", async () => {
    setupPathsCanonical([{ fileName: "global.md", content: "Global rule." }]);
    const provider = new StaticRulesProvider();
  ...
  test("rule with matching paths: is included for that package", async () => {
    setupPathsCanonical([{ fileName: "api.md", content: "API rule.", paths: ["packages/api/**"] }]);
    const provider = new StaticRulesProvider();
    const result = await provider.fetch(MONOREPO_REQUEST);
    expect(result.chunks).toHaveLength(1);
  ...
  test("rule with non-matching paths: is excluded for this package", async () => {
    setupPathsCanonical([{ fileName: "web.md", content: "Web-only rule.", paths: ["packages/web/**"] }]);
    const provider = new StaticRulesProvider();
    const result = await provider.fetch(MONOREPO_REQUEST);
  ...
  test("single-package repo (packageDir === repoRoot) always matches regardless of paths:", async () => {
    ...
    const provider = new StaticRulesProvider();
    // PATHS_BASE_REQUEST has packageDir === repoRoot === "/project"
    const result = await provider.fetch(PATHS_BASE_REQUEST);
    expect(result.chunks).toHaveLength(1);
...
describe("StaticRulesProvider — #558 differentiated empty-merge log", () => {
  ...
  test("returns empty chunks (no fallback to legacy) when repo rules exist but none match package", async () => {
    setupPathsCanonical([{ fileName: "web.md", content: "Web rule.", paths: ["packages/web/**"] }]);
