...
// ─────────────────────────────────────────────────────────────────────────────
// US-003 AC 1, 2, 5, 6: soft-mode warning message wording
// ─────────────────────────────────────────────────────────────────────────────
import type { LogEntry } from "@/logger";
import { addSink, initLogger, resetLogger } from "@/logger";

type WarnCall = { stage: string; message: string; data?: Record<string, unknown> };

function captureWarnCalls(fn: () => Promise<unknown>): Promise<WarnCall[]> {
  const calls: WarnCall[] = [];
  initLogger({ level: "silent", suppressConsole: true });
  const unsubscribe = addSink((entry: LogEntry) => {
    if (entry.level === "warn") calls.push({ stage: entry.stage, message: entry.message, data: entry.data });
  });
  return fn()
    .finally(() => {
      unsubscribe();
...
describe("StaticRulesProvider — US-003 soft-mode warning message wording", () => {
  test("[AC 1] enforceBudget=false: truncation warning states 'would be truncated', names enforceBudget", async () => {
    setupCanonical([
      { fileName: "a.md", id: "a", content: "A".repeat(40), tokens: 200, priority: 1 },
      { fileName: "b.md", id: "b", content: "B".repeat(40), tokens: 200, priority: 2 },
      { fileName: "c.md", id: "c", content: "C".repeat(40), tokens: 200, priority: 3 },
    ]);
    const provider = new StaticRulesProvider({ budgetTokens: 400, enforceBudget: false });
    const calls = await captureWarnCalls(() => provider.fetch(BASE_REQUEST));

    const truncateCalls = calls.filter((c) => c.message.includes("truncated") || c.message.includes("truncat"));
    expect(truncateCalls.length).toBeGreaterThan(0);
    const truncationMsg = truncateCalls.find((c) => c.message.includes("Rule sections"));
    expect(truncationMsg).toBeDefined();
    expect(truncationMsg?.message).toMatch(/would be truncated/i);
    expect(truncationMsg?.message).not.toMatch(/were truncated/i);
    expect(truncationMsg?.message).toContain("enforceBudget");
  });

  test("[AC 2] enforceBudget=true: truncation warning is unchanged from today's text", async () => {
    setupCanonical([
      { fileName: "a.md", id: "a", content: "A".repeat(40), tokens: 200, priority: 1 },
      { fileName: "b.md", id: "b", content: "B".repeat(40), tokens: 200, priority: 2 },
      { fileName: "c.md", id: "c", content: "C".repeat(40), tokens: 200, priority: 3 },
    ...
    const truncationMsg = calls.find((c) => c.message.includes("Rule sections"));
    expect(truncationMsg).toBeDefined();
    expect(truncationMsg?.message).toBe("Rule sections truncated by static rules budget");
    expect(truncationMsg?.message).not.toMatch(/would be truncated/i);
  });

  test("[AC 5] Within budget + enforceBudget=false: emits no truncation warning", async () => {
    setupCanonical([
      { fileName: "a.md", id: "a", content: "A".repeat(40), tokens: 50, priority: 1 },
      { fileName: "b.md", id: "b", content: "B".repeat(40), tokens: 50, priority: 2 },
    ]);
    const provider = new StaticRulesProvider({ budgetTokens: 1000, enforceBudget: false });
    const calls = await captureWarnCalls(() => provider.fetch(BASE_REQUEST));

    const truncateCalls = calls.filter((c) => c.message.includes("truncated"));
    expect(truncateCalls).toHaveLength(0);
  });

  test("[AC 6] Approaching-budget threshold + enforceBudget=false: warning names enforceBudget", async () => {
    // 3 rules of 150 tokens = 450 total, budget 400 → approaching warning
    setupCanonical([
      { fileName: "a.md", id: "a", content: "A".repeat(40), tokens: 150, priority: 1 },
      { fileName: "b.md", id: "b", content: "B".repeat(40), tokens: 150, priority: 2 },
      { fileName: "c.md", id: "c", content: "C".repeat(40), tokens: 150, priority: 3 },
    ]);
    const provider = new StaticRulesProvider({ budgetTokens: 400, enforceBudget: false });
    const calls = await captureWarnCalls(() => provider.fetch(BASE_REQUEST));

    const approachingCalls = calls.filter((c) => c.message.includes("approaching"));
    expect(approachingCalls.length).toBeGreaterThan(0);
    expect(approachingCalls[0].message).toContain("enforceBudget");
    expect(approachingCalls[0].message).not.toMatch(/were truncated|were dropped/i);
  });
});
