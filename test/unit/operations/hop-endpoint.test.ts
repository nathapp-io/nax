...
describe("resolveHopEndpoint()", () => {
  ...
  // Deliberate correction of the pre-refactor behaviour (#1433): the old code
  // reported `modelTier: tier` here even though `modelDef` came from the hop's
  // literal `model` pin, not the tier map. A caller-pinned modelDef being present
  // is irrelevant on these hop kinds since a hop-level literal pin always wins
  // over it; only the reported tier changes.
  test("a swap with a literal pin AND a caller pin reports no tier (#1433 correction)", () => {
    const pin = { provider: "minimax", model: "minimax/MiniMax-M3" };
    const out = resolveHopEndpoint({
      ...base,
      hopKind: { kind: "swap", failure: FAILURE, model: "openrouter/z-ai/glm-5.3-flash[high]" },
  ...
  // Same correction as above, for `timeout-retry`. #1433: a pin of either kind
  // reports no tier, because no tier selected the dispatched model.
  test("a timeout-retry with a literal pin AND a caller pin reports no tier (#1433 correction)", () => {
    const pin = { provider: "minimax", model: "minimax/MiniMax-M3" };
    const out = resolveHopEndpoint({
      ...base,
      hopKind: { kind: "timeout-retry", attempt: 1, model: "openrouter/z-ai/glm-5.3-flash[high]" },
      pinnedModelDef: pin,
    });
    expect(out.modelDef.model).toBe("openrouter/z-ai/glm-5.3-flash[high]");
    expect(out.modelTier).toBeUndefined();
