/**
 * pipeline-result-handler.ts — handlePipelineFailure worktree removal +
 * pause-reason persistence
 *
 * Two concerns, each previously its own satellite, merged per
 * test-architecture.md (split by concern, never by ticket):
 *
 *   BUG-12: worktree removal must drain stdout/stderr and surface non-zero
 *   exits. removeWorktreeDirectory previously awaited `proc.exited` without
 *   consuming stdout/stderr — a git error emitting >64KB fills the pipe
 *   buffer, the child blocks writing, and the handler hangs. Non-zero exits
 *   were also invisible. (BUG-3: the stdout/stderr destructure was swapped;
 *   the regression below asserts the stderr content is what gets logged.)
 *
 *   nax#1582: on a `pause` finalAction, the pipeline reason is appended to the
 *   story's priorErrors as "PAUSED: <reason>" (with the nax#930 fabricated-quote
 *   scrub) instead of leaving priorErrors empty.
 */
...
// ---------------------------------------------------------------------------
// pause-reason persistence (pipeline-result-handler-pause-reason.test.ts)
//
// Split out of pipeline-result-handler.test.ts to stay under the 800-line test
// file cap; merged back here now that the combined failure-side file fits.
// ---------------------------------------------------------------------------

function makePauseCtx(story: UserStory, overrides: Partial<PipelineHandlerContext> = {}): PipelineHandlerContext {
  const prd = makePRD({ userStories: [story] });
  return {
    config: DEFAULT_CONFIG,
    prd,
...
describe("handlePipelineFailure — pause-reason persistence (nax#1582)", () => {
  ...
  test("appends the pipeline reason to priorErrors instead of leaving it empty", async () => {
    ...
    const ctx = makePauseCtx(story, { prdPath });
