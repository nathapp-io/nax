...
describe("useBatch scheduling refresh", () => {
  ...
  // PERF-36 (D-22): the sequential-fallback branch used to recompute the
  // batch plan via `precomputeBatchPlan(getAllReadyStories(prd), 4)`, ignoring
  // any ctx.batchPlan already precomputed in runner-execution.ts. Two
  // sources of truth for batch size + wasted recompute.
  //
  // Build a PRD where the injected batch plan's order differs from what
  // `precomputeBatchPlan(getAllReadyStories(prd), 4)` would produce. If the
  // sequential branch uses ctx.batchPlan, the dispatch order matches the
  // injected plan. If it recomputes, the dispatch order matches the PRD's
  // getAllReadyStories order — which is different.
  test("PERF-36: sequential loop reuses ctx.batchPlan over the hardcoded-4 recompute", async () => {
    const us000 = {
      ...makePendingStory("US-000"),
      // NoTest routing → batchable into a one-slot isBatch=true batch on its
      // own (so injected plan order: [US-noTest-first, US-000, US-001]).
      routing: { complexity: "simple", modelTier: "fast", testStrategy: "no-test", reasoning: "simple" },
    };
    const usNoTest = {
      ...makePendingStory("US-noTest"),
      routing: { complexity: "simple", modelTier: "fast", testStrategy: "no-test", reasoning: "simple" },
    };
    const us001 = {
      ...makePendingStory("US-001"),
    ...
    // PRD order: US-001 first so getAllReadyStories returns it first.
    // If sequential branch recomputes, US-001 is selected first (and
    // US-noTest isn't on its own because PRD order is [US-001, ...]).
    const initialPrd = makePrd([us001, usNoTest, us000]);
    const injectedPlan = precomputeBatchPlan([usNoTest, us000, us001], 4);
    const selectedStoryIds: string[] = [];
    ...
    deps.runIteration = mock(async (_ctx: unknown, prdArg: typeof initialPrd, selection: { story: { id: string } }) => {
      ...
      return { prd: nextPrd, storiesCompletedDelta: 1, costDelta: 0, prdDirty: false };
    });
    ...
    const ctx = {
      ...
      config: {
        ...makeCtx().config,
        execution: { ...makeCtx().config.execution, maxIterations: 3 },
      },
      useBatch: true,
      batchPlan: injectedPlan,
    };

    await executeUnified(ctx as never, initialPrd as never);

    // Injected plan's first non-passed batch slot wins. If the fix is in
    // place, US-noTest is dispatched first (injected plan order).
    // If the fix is absent, sequential branch recomputes with PRD order
    // and US-001 is dispatched first.
    expect(selectedStoryIds[0]).toBe("US-noTest");
  });
});
