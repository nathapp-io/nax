/**
 * Review-decision seam tests.
 *
 * `toReviewDecisionPayload` — regression F3 of
 ...
 *
 * `emitReviewDecision` — every optional field a review op's real verify()
 * populates must survive onto the dispatched `ReviewDecisionEvent`. This is the
 * seam F3 diagnosed for `advisoryFindings`: an op computes a field,
 * `ReviewDecisionEvent` declares it, the audit middleware forwards it — and
 * `emitReviewDecision` alone drops it, so 100% of persisted review-audit records
 * carry it as null. `acDropped`, `acks` and `blockingThreshold` were the same bug,
 * never added to that rescued list (this is the third pass over the seam).
 *
