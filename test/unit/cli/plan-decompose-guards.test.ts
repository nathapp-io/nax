...
const ADAPTER_FEATURE = "cache-feature";

function adapterMakeStory(overrides: Partial<UserStory> = {}): UserStory {
  return {
    id: "US-001",
...
function adapterMakePrd(stories: UserStory[] = [adapterMakeStory()]): PRD {
  return makePRD({ feature: ADAPTER_FEATURE, branchName: `feat/${ADAPTER_FEATURE}`, userStories: stories });
}
...
function adapterMakeConfig(overrides: Partial<NaxConfig> = {}): NaxConfig {
  return { ...makeNaxConfig(), ...overrides };
}

function _adapterMakeFakeScan() {
  return {
    fileTree: "└── src/\n    └── index.ts",
    dependencies: {},
    devDependencies: {},
...
describe("planDecomposeCommand — calls adapter.decompose() not adapter.complete() (US-002 AC-1)", () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(join(tmpDir, ".nax", "features", ADAPTER_FEATURE), { recursive: true });

    const prd = adapterMakePrd();
    const prdPath = join(tmpDir, ".nax", "features", ADAPTER_FEATURE, "prd.json");
  ...
  test("calls adapter.decompose() exactly once", async () => {
    const config = adapterMakeConfig();
    await planDecomposeCommand(tmpDir, config, { feature: ADAPTER_FEATURE, storyId: "US-001" });
  ...
  test("resolves without error when adapter.decompose() is available", async () => {
    const config = adapterMakeConfig();
    expect(
      planDecomposeCommand(tmpDir, config, { feature: ADAPTER_FEATURE, storyId: "US-001" }),
