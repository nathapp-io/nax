...
import { logsCommand, type LogsOptions } from "../../../src/commands/logs";
...
function setupTestProject(featureName: string): string {
	...
	const sampleLogs = [
		{ timestamp: "2026-02-27T10:00:00.000Z", level: "info", stage: "run.start", message: "Starting feature", data: { runId: "run-001", feature: featureName } },
		{ timestamp: "2026-02-27T10:00:01.000Z", level: "info", stage: "story.start", storyId: "US-001", message: "Starting story", data: { storyId: "US-001", title: "Test Story" } },
		{ timestamp: "2026-02-27T10:00:02.000Z", level: "debug", stage: "routing", storyId: "US-001", message: "Routing decision", data: { tier: "haiku" } },
		{ timestamp: "2026-02-27T10:00:03.000Z", level: "info", stage: "story.complete", storyId: "US-001", message: "Story passed", data: { success: true, cost: 0.0023 } },
		{ timestamp: "2026-02-27T10:00:04.000Z", level: "error", stage: "story.start", storyId: "US-002", message: "Story failed", data: { storyId: "US-002", title: "Failed Story" } },
	];
	...
	// Write older run log
	const olderLogs = [
		{ timestamp: "2026-02-26T09:00:00.000Z", level: "info", stage: "run.start", message: "Starting feature", data: { runId: "run-000", feature: featureName } },
		{ timestamp: "2026-02-26T09:00:01.000Z", level: "info", stage: "story.start", storyId: "US-001", message: "Old run", data: { storyId: "US-001", title: "Old Story" } },
	];
	const olderRunPath = join(runsDir, "2026-02-26T09-00-00.jsonl");
	writeFileSync(
		olderRunPath,
		olderLogs.map((log) => JSON.stringify(log)).join("\n"),
	);
