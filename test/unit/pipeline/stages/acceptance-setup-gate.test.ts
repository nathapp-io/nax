...
// ---------------------------------------------------------------------------
// Absorbed: acceptance-setup-strategy.test.ts
// ---------------------------------------------------------------------------

function strategyMakeStory(id: string, acceptanceCriteria: string[]): UserStory {
  return {
    id,
    title: `Story ${id}`,
    description: "desc",
...
function strategyMakePrd(stories: UserStory[]) {
  return {
    project: "test-project",
    feature: "test-feature",
    branchName: "feat/test",
...
function strategyMakeCtx(acceptanceOverrides: Partial<NaxConfig["acceptance"]> = {}): PipelineContext {
  const stories = [strategyMakeStory("US-001", ["renders correctly", "shows expected output"])];
  return {
...
function strategyMakeDefaultCallOp(): typeof _acceptanceSetupDeps.callOp {
  return async (_ctx, _packageDir, op, input) => {
    if (op.name === "acceptance-refine") {
      const { criteria, storyId } = input as { criteria: string[]; storyId: string };
      return criteria.map((c: string) => ({ original: c, refined: c, testable: true, storyId }));
...
describe("acceptance-setup: testStrategy config is consumed (callOp invoked)", () => {
  ...
  test("stage runs and calls callOp when testStrategy='component'", async () => {
    ...
    const ctx = strategyMakeCtx({ testStrategy: "component", testFramework: "ink-testing-library" });
    await acceptanceSetupStage.execute(ctx);
  ...
  test("stage runs and calls callOp when testStrategy='cli'", async () => {
    ...
    const ctx = strategyMakeCtx({ testStrategy: "cli" });
    await acceptanceSetupStage.execute(ctx);
  ...
  test("stage runs and calls callOp when testStrategy is not set in config", async () => {
    ...
    const ctx = strategyMakeCtx(); // no testStrategy
    await acceptanceSetupStage.execute(ctx);
...
describe("acceptance-setup: testFramework appears in generate callOp input", () => {
  ...
  test("frameworkOverrideLine is empty string when testFramework is not set", async () => {
    ...
    const ctx = strategyMakeCtx(); // no testFramework
    await acceptanceSetupStage.execute(ctx);
