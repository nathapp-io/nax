...
import { runPrecheck, EXIT_CODES } from "../src/precheck";
...
async function setupGitRepo(dir: string): Promise<void> {
	...
	await Bun.spawn(["git", "config", "user.email", "test@test.com"], { cwd: dir, stdout: "ignore", stderr: "ignore" }).exited;
...
const createConfig = (workdir: string): NaxConfig =>
	({
		...
		autoMode: { enabled: false, defaultAgent: "test", fallbackOrder: [], complexityRouting: {}, escalation: { enabled: false, tierOrder: [] } },
		quality: { minTestCoverage: 80, requireTypecheck: true, requireLint: true },
		tdd: { strategy: "auto", skipGeneratedVerificationTests: false },
		models: {},
		rectification: { enabled: true, maxRetries: 2, fullSuiteTimeoutSeconds: 120, maxFailureSummaryChars: 2000, abortOnIncreasingFailures: true },
	}) as NaxConfig;
