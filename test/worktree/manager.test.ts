...
describe("WorktreeManager", () => {
	...
	describe("create", () => {
		...
		test("throws descriptive error when worktree already exists", async () => {
			...
			await expect(manager.create(projectRoot, storyId)).rejects.toThrow(
				/already exists|worktree.*exists/i,
			);
		});
	});
	...
	describe("list", () => {
		test("returns active worktree entries", async () => {
			...
			expect(
				paths.some((p) => p.includes(join(".nax-wt", storyId1))),
			).toBe(true);
			expect(
				paths.some((p) => p.includes(join(".nax-wt", storyId2))),
			).toBe(true);
		});
		...
		test("returns empty array when no worktrees exist (except main)", async () => {
			...
			expect(
				worktrees.every((wt) => !wt.path.includes(".nax-wt")),
			).toBe(true);
		});
		...
		test("each entry contains path and branch info", async () => {
			...
			const worktrees = await manager.list(projectRoot);
			const ourWorktree = worktrees.find((wt) =>
				wt.path.includes(join(".nax-wt", storyId)),
			);
