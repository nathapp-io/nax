...
/**
 * Recover the ABSOLUTE repo root from the story's ABSOLUTE package dir and its
 * repo-relative package path.
 *
 * `packageDirAbs === join(repoRootAbs, workdirRel)` (PipelineContext contract,
 * src/pipeline/types.ts), so walking back up `workdirRel`'s own segment count
 * recovers the root without a second field on StoryContext. Root stories
 * (`workdirRel === "."`) are already at the root.
 */
function repoRootFromPackage(packageDirAbs: string, workdirRel: string): string {
  return workdirRel === "." ? packageDirAbs : path.resolve(packageDirAbs, ...workdirRel.split("/").map(() => ".."));
}
...
async function addFileElements(
  ...
  // nax single-frame redesign PR 2: the agent's tools are rooted at the repo
  // root post-move, so every repo-rooted declared path is reachable by
  // construction -- package reframing and the plan-time-absent reclassification
  // it depended on are retired for RENDERING purposes here. `contextFiles`
  // (the story's own entries plus merged parent outputs) and `expectedFiles`
  // pass straight through as stored; nothing is dropped as unreachable.
  const framedContextFiles = contextFiles;
  const framedExpectedFiles = expectedFiles;
  const unreachable: string[] = [];

  // Paths are repo-rooted, so disk resolution must be too. `workdir` is the
  // ABSOLUTE package dir; `repoRootFromPackage` walks back the story's package
  // segments, while a root story resolves directly against `workdir`.
  const repoRoot = repoRootFromPackage(workdir, workdirRel);

  if (unreachable.length > 0) {
    // Dead branch after PR 2's pass-through (`unreachable` is always `[]`);
    // retained for PR 4 to remove alongside the retired frame helpers. §9
    // (.nax/rules/monorepo-awareness.md): storyId first, packageDir present,
    // so a parallel run's JSONL is attributable. Vocabulary per that rule:
    // `packageDir` is the ABSOLUTE package dir, `workdir` the RELATIVE
    // story.workdir.
    _contextBuilderDeps
      .getLogger()
  ...
  await addCreateIntentElements(elements, repoRoot, framedExpectedFiles, surfaced);
}
