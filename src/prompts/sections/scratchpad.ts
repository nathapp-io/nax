...
Its contents are wiped when a run finishes and are never committed. A failed run's scratchpad is retained
for inspection — files an agent wrote before failing survive that run and are cleared at the next run's
start. Nothing there survives the run, reaches the repository, or is read by another step — treat every
file as disposable, and overwrite freely.
