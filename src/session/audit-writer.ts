/**
 * Prompt Audit Writer — fire-and-forget file writer for agent prompts.
 *
 * Moved from src/agents/acp/prompt-audit.ts so all audit logic lives under
 * src/session/. The ACP adapter no longer owns audit policy — it reports
 * prompt turns via a callback; SessionManager enriches and delegates here.
 *
 * File layout:
...
export interface PromptAuditEntry {
  ...
  resumed?: boolean;
  /**
   * Stable nax session ID (sess-<uuid>). Populated by SessionManager.auditPrompt() so
   * audit files can be correlated across agent swaps and reconnects using a key that
   * never changes for the lifetime of a logical session.
   */
 ...
 * Call with `void writePromptAudit(...)` (fire-and-forget) from audit policy layer.
 */
