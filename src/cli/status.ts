...
import { loadPRD, countStories } from "../prd";
...
async function displayAllFeatures(projectDir: string): Promise<void> {
	...
	const summaries = await Promise.all(
		features.map((name) => getFeatureSummary(name, join(featuresDir, name))),
	);
	...
	for (const summary of summaries) {
		...
		const failed = summary.failed > 0 ? chalk.red(String(summary.failed).padEnd(8)) : chalk.dim(String(summary.failed).padEnd(8));
