...
function detectFailurePattern(
	...
	if (
		story.status === "passed" &&
		story.priorErrors?.some((err) =>
			err.toLowerCase().includes("greenfield-no-tests"),
		)
	) {
	...
	const testFailingCount = story.priorErrors?.filter((err) =>
		err.toLowerCase().includes("tests-failing"),
	).length || 0;
	...
	if (
		story.priorErrors?.some((err) => err.toLowerCase().includes("ratelimited"))
	) {
		return "RATE_LIMITED";
	}
	...
	if (
		story.priorErrors?.some((err) => err.toLowerCase().includes("session-failure"))
	) {
...
function generateRecommendations(report: DiagnosisReport): string[] {
	...
	// Rate limiting
	const rateLimited = report.failureAnalysis.filter((f) =>
		f.pattern === "RATE_LIMITED",
	);
	...
	// Greenfield issues
	const greenfield = report.failureAnalysis.filter((f) =>
		f.pattern === "GREENFIELD_TDD",
	);
	if (greenfield.length > 0) {
		recommendations.push(
			"Consider adding --greenfield flag or bootstrap tests for greenfield stories",
		);
	}
...
export async function diagnoseCommand(options: DiagnoseOptions = {}): Promise<void> {
	...
	// Count commits
	const commitCount = await countCommitsSince(
		projectDir,
	...
	for (const story of prd.userStories) {
		...
		if (
			story.status === "failed" || story.status === "blocked" || story.status === "paused" || pattern === "AUTO_RECOVERED"
		) {
