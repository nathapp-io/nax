...
/**
 * What the message builder reads off the phase's completed nodes.
 *
 * Replaces the acpx `ctx.outputs` map this was ported from (keyed by node id,
 * read via `outputsFor`/`findingsFor`). A struct carrying only findings would
 * silently drop the failing-gate list and the acceptance runner evidence, so
 * this names all three shapes the original read: `review_<phase>.findings`,
 * `quality_gates.{failing,output}`, and `acceptance.output`.
 */
...
/** Options carrying what the message builder cannot read off `ctx`. */
...
function findingsFor(ctx: CommitMessageCtx, _phase: FinishPhase): Finding[] {
  const raw = ctx.findings;
...
function subjectFor(phase: FinishPhase, ctx: CommitMessageCtx): string {
  if (phase === "gate") {
    const failing = ctx.gate?.failing ?? [];
...
function bodyFor(phase: FinishPhase, ctx: CommitMessageCtx, opts: MessageOptions): string[] {
  if (phase === "gate") {
    const failing = ctx.gate?.failing ?? [];
    const evidence = runnerEvidence(ctx.gate?.output ?? "", opts);
    return [...(failing.length > 0 ? [`Failing: ${failing.join(", ")}`] : []), ...(evidence ? [evidence] : [])];
  }
  if (phase === "acceptance") {
    const evidence = runnerEvidence(ctx.acceptance?.output ?? "", opts);
