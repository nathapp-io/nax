...
### 0.1 Count `nonNullAssert` with biome, not the regex

**The regex counter is not the drain's finish line. Biome is.** Re-measured 2026-08-26 on
`main`:
...
So 272 non-null assertions in `test/` are counted by **nothing**: the regex misses them and
`noNonNullAssertion` is `off` for `test/**`. This matters for endgame item 4 specifically —
draining `nonNullAssert` to 0 as the regex measures it leaves those sites live, and the
...
Swap the final `Counter` for one keyed on `x['location']['path']` to get the per-file ranking
that §1's queue is built from.
...
```bash
# 1. see this unit's sites (biome, not grep — the regex undercounts, §0.1)
bun x @biomejs/biome@2.5.10 check --config-path=/tmp/biome-probe <path> --reporter=json 2>/dev/null
...
Commit as `test: <what>` with a body line carrying the biome delta, e.g. `nonNullAssert: P → Q`.

## 4. Forbidden — these lower the number without doing the work

- Adding `as any`, `@ts-ignore`, `@ts-expect-error`, `@ts-nocheck`, `as never`, or
  `as unknown as`.
- Adding `// test-ratchet-allow: …` or a `// biome-ignore lint/style/noNonNullAssertion` line.
- Replacing `x!` with `x as X`, `x as NonNullable<typeof x>`, or any other cast — that trades
  `nonNullAssert` for `looseCast` and asserts strictly less.
...
These cost real time to learn and apply unchanged. Full accounts in
`archive/STATUS-1514-typecheck-drain.md` and `archive/LOG-no-explicit-any-drain.md` at the
sections named.
...
- **A defensive `?.` is not evidence of a tolerated absence** (§44). If the schema carries
  ...
  impossible state. **This one is load-bearing for `noNonNullAssertion`** — most `!` sites are
...
- **An accepted exception is a ruling, not a law** (§47). A tier-3 "undrainable" ruling turned
  ...
  satisfied the slot.
- **"Every route out trades a counter" is a survey, not a proof** (§1, §8.13). Three passes
...
- **A "src-blocked" ruling names a seam the author had in mind, not the seam the error
  demands** (archive §8.25). Re-derive which property of the site rejects the test; the held
...
- **Reproduce against the project's own script** (§8.13), not a hand-rolled invocation of the
...
| Doc | Holds |
|:--|:--|
| `archive/LOG-no-explicit-any-drain.md` | §8.14–§8.25 — the closed `noExplicitAny` drain, twelve batches, 235 files, biome 1529 → 0 |
...
| `archive/HANDOFF-cast-drain-batch1.md` | the `as unknown as` batch-1 brief (its log is still §8.1–§8.13 below) |
...
**§8.1–§8.13 below are the closed `as unknown as` drain** (101 → 0). They stay in the live doc
because §1's central ruling — "every route out trades a counter" was a survey, not a proof —
was earned there and is the one most likely to be re-derived wrongly during the
`noNonNullAssertion` drain. Archive them once that drain closes.
