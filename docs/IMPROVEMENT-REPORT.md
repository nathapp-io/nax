...
| Rule-based prompt optimizer is real working code but off by default and undiscoverable | `src/optimizer/rule-based.optimizer.ts`, `src/optimizer/index.ts` | Resolved 2026-08-20 — removed. No config ever selected it, and its `optimizer.strategies` knobs were absent from `OptimizerConfigSchema`, so it was unconfigurable even when opted into. `optimizer.strategy`/`strategies` now stripped with a warning. |
