...
**Status:** Phases A–F complete; Phase 3.5 complete (2026-04-27)
**Date:** 2026-04-26
...
| **3.5** | Adapter method deletion (release gate) | Done | — | C soaked |
...
- [x] `plan()`, `decompose()` deleted from `AgentAdapter`; `planAs()`, `decomposeAs()` deleted from `IAgentManager`
- [x] `src/cli/plan.ts` migrated from `planAs()` to `runAs()`
- [x] Deprecated test files deleted (`plan.test.ts`, `decompose.test.ts`, `adapter-phase3.test.ts`)
- [x] Dead `planAs`/`decomposeAs` stubs removed from 20+ test files and mock helpers
- [x] `adapter-boundary.test.ts` ALLOWED_PATTERNS updated
- [x] `bun run typecheck` clean
