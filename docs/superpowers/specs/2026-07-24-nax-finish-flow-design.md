...
3. **Review = two isolated acpx `acp` calls, each agent/model-pinned.** `review_spec` and `review_quality` are **separate isolated `acp` nodes** (own session each), each pinning its own reviewer via the node's `profile` field. acpx resolves `profile` per-node at runtime against the `agents` map in `~/.acpx/config.json` (model encoded in that agent's `args`); unpinned nodes fall back to `--default-agent`. The flow module loads fresh per `acpx flow run`, so each review node reads its profile from an env var (`NAX_FINISH_SPEC_PROFILE`/`NAX_FINISH_QUALITY_PROFILE`) the plugin sets from nax config — distinct spec/quality reviewers are fully config-driven with **no acpx-fork change**. This recovers `post-impl-review`'s isolated-subagent property and lets each lens run with its own reviewer.
