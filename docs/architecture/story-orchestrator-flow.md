...
**Story Orchestrator** (`src/execution/story-orchestrator/` — a directory; `index.ts` is
the public barrel), which executes an ordered list of **phases** and, on failure, drives a
**rectification (fix) cycle** (`src/findings/cycle.ts`) before deciding the story's verdict.

```
executionStage  [src/pipeline/stages/execution.ts]
  → assemblePlanInputsFromCtx()      [src/execution/plan-inputs.ts]              — builds the typed per-phase inputs
  → buildPlanForStrategy()           [src/execution/build-plan-for-strategy.ts]  — StoryOrchestratorBuilder → ExecutionPlan
  → ExecutionPlan.run()              [story-orchestrator/execution-plan.ts]      — runs phases in CANONICAL_ORDER
      → runPhase() per phase, short-circuit on failure          [story-orchestrator/run-phase.ts]
      → on failure: runRectification() → runFixCycle()          [story-orchestrator/rectification.ts, src/findings/cycle.ts]
      → post-rectification resume + staleness guard → final verdict
```

| File (under `src/execution/story-orchestrator/`) | Owns |
|:---|:---|
| `types.ts` | `CANONICAL_ORDER`, `STRATEGY_TO_REVALIDATION_PHASES`, `EXHAUSTED_EXIT_REASONS`, result types |
| `builder.ts` | `StoryOrchestratorBuilder` (`addX()` per phase, `build()` → `ExecutionPlan`) |
| `execution-plan.ts` | `ExecutionPlan.run()` — main loop, resume loops, verifier-SSOT carve-out, final verdict |
| `rectification.ts` | `runRectification()` — wraps `runFixCycle` with the revalidation sweep |
| `phase-eval.ts` | `phasePassed()`, `phasesToRevalidate()`, `orderGateLast()`, gate-regression detection |
| `run-phase.ts` | `runPhase()` — dispatches one op (`_storyOrchestratorDeps`) |
...
`src/execution/story-orchestrator/types.ts` — `CANONICAL_ORDER`
...
const CANONICAL_ORDER: readonly PhaseKind[] = [
  ...
  "test-presence-gate", // deterministic gate (single-session tdd-simple / test-after only)
  "full-suite-gate",    // deterministic gate
  "mutation-check",     // advisory deterministic check (opt-in)
...
| **Deterministic gate** | `greenfield-gate`, `test-presence-gate`, `full-suite-gate`, `verify-scoped` | No LLM — filesystem scan or test-runner execution |
| **Advisory check** | `mutation-check` | No LLM — applies a few mutants and runs scoped tests; always `success: true` |
...
Gate one-liners:
- **greenfield-gate** — runs *after* the test-writer: a filesystem scan (tracked **and** untracked, `.nax/` excluded) for test files. None found → `success: false`, `pauseReason: "greenfield-no-tests"`, which escalates so tier escalation can switch the story to `test-after`. `src/operations/greenfield-gate.ts`
- **test-presence-gate** — the single-session counterpart: runs after the implementer for `tdd-simple` / `test-after` (not `no-test`) and fails with `pauseReason: "no-tests-authored"` if the implementer wrote no test file; the story escalates with an explicit "you MUST write tests" directive in `priorErrors`. `src/operations/test-presence-gate.ts`
- **full-suite-gate** — runs the full test suite, returns structured pass/fail findings. `src/operations/full-suite-gate.ts`
- **mutation-check** — opt-in (`execution.mutationCheck.enabled`, default `false`; `maxMutants` 3, `timeoutSeconds` 60) mutation spot-check after GREEN. Advisory only — never short-circuits; the worktree is always restored. `src/operations/mutation-check.ts`
...
`isThreeSessionStrategy()` in `src/config/test-strategy.ts`:
...
(`src/execution/build-plan-for-strategy.ts`), keyed off
`isThreeSession = isThreeSessionStrategy(testStrategy)` and
`freshRun = isFreshRun(story)`:

```typescript
if (isThreeSession && freshRun && inputs.testWriter)     builder.addTestWriter(...);
if (isThreeSession && freshRun && inputs.greenfieldGate) builder.addGreenfieldGate(...);
if (inputs.implementer)                                  builder.addImplementer(...);     // always present
if (!isThreeSession && inputs.testPresenceGate)          builder.addTestPresenceGate(...);
// regressionMode = config.execution.regressionGate.mode (default "deferred")
if (inputs.fullSuiteGate && (isThreeSession || regressionMode === "per-story"))
                                                          builder.addFullSuiteGate(...);
if (inputs.mutationCheck)                                builder.addMutationCheck(...);   // execution.mutationCheck.enabled
if (isThreeSession && inputs.verifier)                   builder.addVerifier(...);
if (!isThreeSession && inputs.verifyScoped)              builder.addVerifyScoped(...);
...
```
[test-writer]  → greenfield-gate → [implementer] → full-suite-gate → (mutation-check?)
...
```
[implementer] → test-presence-gate* → [full-suite-gate?] → (mutation-check?) → verify-scoped
→ lint-check → typecheck-check → semantic-review → adversarial-review
```

```mermaid
flowchart TD
    IMP(["implementer<br/>single agent session —<br/>owns tests + code"]) --> TPG{{"test-presence-gate<br/><i>tdd-simple / test-after only</i>"}}
    TPG --> FSG{{"full-suite-gate<br/><i>only if regressionMode = per-story</i>"}}
...
- `test-presence-gate` (\*) runs only for the test-owning strategies `tdd-simple` / `test-after` (`isSingleSessionTestOwningStrategy()`).
- `full-suite-gate` runs only when `execution.regressionGate.mode === "per-story"` (default `"deferred"`); otherwise scoped verification is the test gate.
- `mutation-check` (both modes) is present only when `execution.mutationCheck.enabled` is `true`.
...
`src/config/runtime-types.ts` — `TddConfig.sessionTiers`:
...
(`src/execution/story-orchestrator/types.ts`). So "what re-runs after a fix?" depends on
...
> (`story-orchestrator/types.ts`); an unknown/plugin strategy falls back to re-running **all** phases.
...
`phasesToRevalidate()` (`src/execution/story-orchestrator/phase-eval.ts`): if `strategiesRun`
...
- Exit reasons (`EXHAUSTED_EXIT_REASONS`, `src/execution/story-orchestrator/types.ts`):
...
- **Terminal lite-validate**: when a strategy exhausts its attempts, a final "lite"
  validate re-orders phases so `full-suite-gate` runs **last** (`orderGateLast()`, `phase-eval.ts`).

### Post-rectification resume & staleness guard

All in `ExecutionPlan.run()` (`src/execution/story-orchestrator/execution-plan.ts`):

- **Resume loop**: after the cycle exits, the orchestrator walks the canonical
  order again and runs any phase that has **not** already passed. A phase already in
  `phaseOutputs` that passed is skipped:
  `if (name in phaseOutputs && phasePassed(...)) continue;`. A *fresh*
  never-before-seen failure triggers one additional second rectification pass per story.
- **Mechanical-only resume**: when rectification exhausts with only `lint` / `typecheck`
  findings left, phases that never executed (typically the two reviews) still run, so a
  story cannot pass without semantic/adversarial judgment. This loop skips every phase
  already in `phaseOutputs`, passed or failed (`if (name in phaseOutputs) continue;`).
- **Verifier-SSOT carve-out**: once `verifier` explicitly passes, later
  `full-suite-gate` failures are treated as unrelated regressions and do not block — but
  the **staleness guard** re-fails the story if rectification introduced *new*
  gate failures (post-hoc detection, not re-execution; surfaced as `gateRegressedDuringRect`).
- **Completeness guard**: a configured `semantic-review` / `adversarial-review` that never
  ran (e.g. the resume loop broke at a still-red gate) forces `success: false` and is
  reported in `missingRequiredReviewPhases` → failure category `review-incomplete`.
...
- `mechanical-lintfix → ["lint-check"]` only (`STRATEGY_TO_REVALIDATION_PHASES`) — the validate callback re-runs
  lint-check alone.
- `full-suite-gate` passed earlier (canonical position 5), so it sits in `phaseOutputs`
  as passed. The resume loop skips it, and the mechanical-only resume path also
  skips it (`if (name in phaseOutputs) continue;`).
- The staleness guard only *detects* a gate regression post-hoc; it does **not**
  re-execute the gate.

**Implication:** the soundness of `mechanical-lintfix → ["lint-check"]` rests entirely on
the assumption documented in the code comment above `STRATEGY_TO_REVALIDATION_PHASES`:
...
|:---|:---|:---|
| What selects three vs single session? | `routing.testStrategy` via `isThreeSessionStrategy()` | `src/config/test-strategy.ts` |
| Where is the phase list assembled? | `buildPlanForStrategy()` | `src/execution/build-plan-for-strategy.ts` |
| Canonical phase order | `CANONICAL_ORDER` | `src/execution/story-orchestrator/types.ts` |
| What re-runs after a fix? | `STRATEGY_TO_REVALIDATION_PHASES` | `src/execution/story-orchestrator/types.ts` |
| Conservative re-run fallback | `phasesToRevalidate()` | `src/execution/story-orchestrator/phase-eval.ts` |
| Cycle exit reasons | `EXHAUSTED_EXIT_REASONS` | `src/execution/story-orchestrator/types.ts` |
| Final verdict, resume, carve-out | `ExecutionPlan.run()` | `src/execution/story-orchestrator/execution-plan.ts` |
| Implementer model tier | `story.routing.modelTier` (not `sessionTiers.implementer`) | `src/config/runtime-types.ts` |
