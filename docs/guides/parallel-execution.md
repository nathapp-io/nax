...
> **`--parallel 0`:** the CLI help and `RunOptions` describe `0` as "auto", but no auto-detection is implemented — the executor only dispatches batches when `n > 0`, so `--parallel 0` currently runs sequentially. Pass an explicit count.
