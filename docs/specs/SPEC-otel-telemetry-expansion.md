...
| Queue exceeds `maxQueueSize` | Drop oldest entries, report an accurate drop count |
...
- US-005 only: de-duplicating spans when two processes export under the same run id is deferred.
- US-006 only: persisting heartbeat state across a process restart is deferred; a restarted run emits a new series.
...
6. **US-006: Incremental OTLP export — batch queue, traceparent, span tree** — depends on US-005
7. **US-007: Heartbeat gauges, histograms, redaction, flush lifecycle** — depends on US-006

US-002 and US-003 are a sizing split of one emission concern: US-002 lands the event and its outcome derivation, US-003 adds the `phaseTelemetry` context slice and the `PhaseDetails` payloads that ride on it. Combined they exceed the per-story AC cap.
...
- **SEAM-4 (US-006 -> US-007):** the batch queue's `flushNow` must be invoked by the reporter's `onRunEnd`, including on the abnormal-exit path where no `run:completed` event reaches the bus.
...
- `test/unit/config/schemas-reporters.test.ts` — existing config-default test patterns
...
- `test/unit/execution/story-orchestrator/run-phase.test.ts` — existing `runPhase` test patterns
...
### US-006: Incremental OTLP export — batch queue, traceparent, span tree

Replace the end-of-run flush with a bounded batch queue, add W3C traceparent adoption, and build the run/story/phase span tree so spans export as work completes.

**Context Files**
- `src/plugins/builtin/otel-reporter/index.ts` — reporter to rebuild around the queue and tree
- `src/plugins/builtin/otel-reporter/otlp.ts` — existing OTLP payload builders
...
- `test/unit/plugins/builtin/otel-reporter/index.test.ts` — existing golden-payload patterns
...
### US-007: Heartbeat gauges, histograms, redaction, flush lifecycle

Add the periodic heartbeat that makes in-flight runs visible, the histogram payload builder, the detail-level redaction boundary, and the flush lifecycle that survives abnormal exit.

**Context Files**
- `src/plugins/builtin/otel-reporter/index.ts` — reporter lifecycle hooks and teardown
- `src/plugins/builtin/otel-reporter/otlp.ts` — metric payload builders to extend
- `src/plugins/builtin/otel-reporter/batch-queue.ts` — created by US-005, flushed here
...
- [unit] An `adversarial-review` operation emits `details.blocking` counts matching its normalized findings grouped by severity.
...
- [unit] `flushNow` completes without throwing when the export request and its retry both fail.
...
- [unit] With `detail` set to `"counts"`, no exported payload contains any review finding's title text.
- [unit] With `detail` set to `"verbose"`, an exported payload contains a span event carrying a review finding's title.
