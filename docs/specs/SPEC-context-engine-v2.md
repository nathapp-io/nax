...
7. **Packing correctness.** Phase 0–2: greedy packing (sort by `score/tokens` descending, floor first) plus the `best-of(greedy, largest feasible single item)` repair is a **½-approximation** of the 0/1 optimum — not a 5%-of-optimal guarantee, as this assumption claimed until 2026-08-10. It reaches ≥95% of the optimum only inside the repair envelope AC-4's property test covers: cases where either every item fits, or a single feasible item is the optimum. Outside that envelope the shortfall can approach 50% — `A(51 tok, score 52)`, `B(50, 50)`, `C(50, 50)` at budget 100 packs `[A]` = 52 against an optimum of `[B,C]` = 100, a 48% shortfall. That is an accepted trade: real candidate sets are 5–20 chunks with no adversarial structure, and closing the gap means exact 0/1 DP. Phase 3+: if 0/1 DP knapsack is added, it maximizes total score subject to the budget constraint.
