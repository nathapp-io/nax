...
The `custom` strategy loads a user-provided module that exports `IPromptOptimizer`:
...
module.exports = {
  ...
  async optimize(input) {
    // Call external API, local model, LLMLingua, etc.
...
The loader validates the module exports `name` (string) and `optimize` (function) before use. Falls back to NoopOptimizer if validation fails.
