...
| `new AgentManager(config)` / `createAgentManager(config, …)` outside `src/runtime/internal/` | `createRuntime(config, workdir).agentManager` | ADR-018 §2 — one runtime per run, no orphans |
