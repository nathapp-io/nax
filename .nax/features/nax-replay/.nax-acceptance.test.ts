...
import { runReplay } from "../../../src/commands/replay";
import { _discoveryDeps } from "../../../src/replay/discovery";
import { _replayCmdDeps } from "../../../src/commands/replay";
...
describe("US-001 inferPhases", () => {
  ...
  test("AC-2: single passing phase returns array with correct name and status", () => {
    ...
    expect(result.phases[0]!.name).toBe("implementer");
  ...
  test("AC-3: multiple passing phases preserve chronological order", () => {
    ...
    const result = inferPhases(entries, STORY_ID);
    expect(result.phases.map((p: { name: string }) => p.name)).toEqual(["test-writer", "implementer"]);
  ...
  test("AC-4: failed phase entry has status fail", () => {
    ...
    const result = inferPhases(entries, STORY_ID);
    expect(result.phases.some((p: { name: string; status: string }) => p.name === "full-suite-gate" && p.status === "fail")).toBe(true);
  });
...
describe("US-001 reconstructTimeline", () => {
  test("AC-6: reconstructTimeline importable with arity >= 1", () => {
    expect(typeof reconstructTimeline).toBe("function");
    expect(reconstructTimeline.length).toBeGreaterThanOrEqual(1);
  });
  ...
  test("AC-7: returns RunTimeline with runId, feature, and matching stories length", () => {
    ...
    const meta = { runId: runMetrics.runId, feature: runMetrics.feature };
    const result = reconstructTimeline({ entries: logSpine, runMetrics, meta });
    expect(result.runId).toBe(runMetrics.runId);
    expect(result.feature).toBe(meta.feature);
  ...
  test("AC-8: story timeline enriched from StoryMetrics", () => {
    ...
    const meta = { runId: runMetrics.runId, feature: "f" };
    const result = reconstructTimeline({ entries: logSpine, runMetrics, meta });
  ...
  test("AC-9: null metrics with crash signal returns status crashed and undefined costs", () => {
    ...
    const status = { run: { status: "crashed" as const } } as any;
    const result = reconstructTimeline({ entries: logSpine, status });
  ...
  test("AC-11: returned RunTimeline has inferred === true", () => {
    ...
    const logSpine = [makeLogEntry({ stage: "run.start", message: "started", data: { runId: runMetrics.runId } })];
    const result = reconstructTimeline({ entries: logSpine, runMetrics, meta: { runId: runMetrics.runId, feature: "f" } });
...
describe("US-003 renderReport", () => {
  ...
  test("AC-25: report header contains runId, feature, status, story count, and cost", () => {
    const timeline = makeTimeline({
      ...
      status: "failed",
      stories: [
        { storyId: "A", status: "passed", phases: [], cost: 0.01, finalTier: "balanced", attempts: 1 },
        { storyId: "B", status: "passed", phases: [], cost: 0.02, finalTier: "balanced", attempts: 1 },
        { storyId: "C", status: "failed", phases: [], cost: 0.02, finalTier: "balanced", attempts: 2 },
      ],
    });
    const output = renderReport(timeline as any);
...
describe("US-004 runReplay seams", () => {
  ...
  const mockDiscovered = {
    ...
    jsonlPath: "/nonexistent/run.jsonl",
  } as any;
  ...
  beforeEach(() => {
    saved = {
      discoverRun: _replayCmdDeps.discoverRun,
      renderReport: _replayCmdDeps.renderReport,
      toReplayJson: _replayCmdDeps.toReplayJson,
      stdout: _replayCmdDeps.stdout,
      stderr: _replayCmdDeps.stderr,
      readJsonl: _replayCmdDeps.readJsonl,
      readMetrics: _replayCmdDeps.readMetrics,
      readStatus: _replayCmdDeps.readStatus,
    };
    _replayCmdDeps.readJsonl = async () => [];
    _replayCmdDeps.readMetrics = async () => undefined;
    _replayCmdDeps.readStatus = async () => undefined;
  });

  afterEach(() => {
    Object.assign(_replayCmdDeps, saved);
  });

  test("AC-30: runReplay calls discoverRun exactly once with the query string", async () => {
    let callCount = 0;
    let callArg: unknown;
    _replayCmdDeps.discoverRun = async (q: string) => { callCount++; callArg = q; return mockDiscovered; };
    _replayCmdDeps.renderReport = () => "ok";
  ...
  test("AC-31: runReplay calls renderReport once with the reconstructed timeline", async () => {
    _replayCmdDeps.discoverRun = async () => mockDiscovered;
    let renderCount = 0;
    let renderArg: unknown;
    _replayCmdDeps.renderReport = (tl: any) => { renderCount++; renderArg = tl; return "ok"; };
  ...
  test("AC-32: --json path calls toReplayJson, writes JSON output, skips renderReport", async () => {
    ...
    _replayCmdDeps.toReplayJson = (tl: any) => { toJsonCount++; return { runId: tl.runId }; };
    _replayCmdDeps.renderReport = () => { renderCount++; return ""; };
    _replayCmdDeps.stdout = (s: string) => outputParts.push(s);
  ...
  test("AC-35: runReplay returns exit code 1 and writes message containing query for unknown run", async () => {
    _replayCmdDeps.discoverRun = async () => {
      throw new NaxError("Run not found: missing", "RUN_NOT_FOUND", { stage: "discovery" });
    };
    const errParts: string[] = [];
    _replayCmdDeps.stderr = (s: string) => errParts.push(s);
  ...
  test("AC-36: malformed JSONL lines skipped; renderReport called once with valid timeline", async () => {
    ...
    try {
      ...
      _replayCmdDeps.discoverRun = async () => ({
        meta: { ...mockDiscovered.meta, runId: "r", feature: "f" },
        jsonlPath,
      } as any);
      _replayCmdDeps.readJsonl = async (p: string) => {
      ...
      _replayCmdDeps.renderReport = (tl: any) => { renderCount++; capturedTimeline = tl; return "ok"; };
  ...
  test("AC-37: crashed run writes CRASHED report and returns exit code 0", async () => {
    ...
    const outputParts: string[] = [];
    _replayCmdDeps.stdout = (s: string) => outputParts.push(s);
    _replayCmdDeps.renderReport = (tl: any) => `CRASHED ${tl.runId}`;
    const exitCode = await runReplay("query", {});
    expect(exitCode).toBe(0);
