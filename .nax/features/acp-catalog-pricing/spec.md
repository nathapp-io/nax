...
- **US-001** `.nax/rules/adapter-wiring.md` — the "Native path" section asserts
  ...
  `src/agents/catalog/` as the second. Regenerate the per-agent shims with `nax rules export --agent=<id>`.
...
**Verification note.** Removals are verified by the build/static gate, not by acceptance
