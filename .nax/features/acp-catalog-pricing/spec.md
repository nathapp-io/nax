...
- **US-001** `.nax/rules/adapter-wiring.md` — the "Native path" section asserts
  `src/agents/native/` is the only directory that may import `@nathapp/nax-ai`; it must name
  `src/agents/catalog/` as the second. Edit this canonical file only. Do **not** run
  `nax rules export`, and do not edit the generated per-agent shims (`CLAUDE.md`, `AGENTS.md`,
  `GEMINI.md`, `codex.md`) by hand — regenerating them is a manual maintainer step outside this
  story.
...
**Verification note (US-001).** After the `.nax/rules/adapter-wiring.md` edit,
`bun run check:rules-drift` reports the per-agent shims as out of date. This is **expected and
correct** — the shims are regenerated by the maintainer, not by this story. The check is not
part of `bun run lint` (the configured quality gate), so it does not fail the story; it does run
inside `bun run check:all`, which `.githooks/pre-commit` and CI both invoke, so the drift must be
cleared by the maintainer's manual `nax rules export --agent=<id>` before the branch lands.
Do not clear it by reverting the rule edit or by running the export from inside the story.
