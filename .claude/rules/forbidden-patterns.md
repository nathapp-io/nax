...
| Setting both `op.hopBody` and `op.retry` on a `RunOperation` | Choose exactly one: either `hopBody` for intra-hop multi-turn orchestration, or `retry` for parse-validation retry. | Coexistence causes silent precedence (hopBody wins), leading to unexpected behavior where retry is ignored. `callOp` guards with `OP_HOPBODY_RETRY_BOTH_SET` (US-004). |
